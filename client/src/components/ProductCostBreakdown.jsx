...
export default function ProductCostBreakdown({ initialBreakdown, onClose, onSubmit, totalCost }) {
  ...
  const handleOpenCreditNotePopup = async (itemId, supplierIndex) => {
    ...
    try {
      ...
      console.log("API response for available notes:", response.data.data); 
  ...
  const handleSupplierChange = (itemId, supplierIndex, field, value) => {
    setBreakdown(prev => prev.map(item => {
      ...
      // Recalculate only after state has been updated to reflect new values
      recalculateSupplierAndItem(updatedItem, itemId, supplierIndex);
      return updatedItem;
    }));
  };
            ...
            {breakdown.map((item) => (
              <div key={item.id} className="border p-4 rounded-lg bg-gray-50">
                <div className="flex items-center space-x-2 mb-2">
                    <input type="text" value={item.category} onChange={e => handleCategoryChange(item.id, 'category', e.target.value)} placeholder="Category name (e.g., Flight)" className="flex-1 p-2 border rounded-lg bg-white"/>
                ...
                <div className="ml-4 space-y-2">
                  {item.suppliers.map((s, index) => {
                    ...
                    // NEW: Determine if the method involves credit notes
                    ...
                    // NEW: The amount the user intends to cover with notes
                    ...
                    <div key={index} className="border-t pt-3 mt-2 space-y-2">
                      ...
                      {/* CHANGED: Simplified rendering logic for inputs */}
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                        { s.paymentMethod.includes('_AND_') ? (
                          <>
                            ...
                            {/* Always show an input for the second amount */}
                           ...
                           /* Always show an input for the single amount */
                      ...
                      {/* CHANGED: Logic for showing credit note section and disabling button */}
                      {involvesCreditNotes && (
                        <div className="mt-2 p-3 bg-blue-50 rounded-lg">
                           ...
                           <button
                             ...
                             disabled={amountToCoverByNotes <= 0} // The button is disabled until the user enters an amount to cover
                           >
                             Select Credit Notes
                           </button>
                        </div>
        ...
        <SelectCreditNotesPopup
          amountToCover={
            (() => {
                ...
                if (!s) return 0;
                // NEW: Simplified logic for passing amount to popup
